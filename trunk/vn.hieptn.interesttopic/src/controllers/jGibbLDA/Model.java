...
//                        // test
//			for(int j=0;j<10;j++){
//			System.out.print("theta[1][K]:"+theta[1][j]+"\n");
//			}
//			for(int i=0;i<10;i++){
//                            System.out.print("theta[M][1]:"+theta[i][1]+"\n");
//                        }
        ...
        public boolean saveModelThetaDB(){
		try{
                    for (int i = 0; i < M; i++){
                        for (int j = 0; j < K; j++){
                            ClassifiedMessageEnt msgEnt = new ClassifiedMessageEnt();
                            msgEnt.PostId = ConfigInfo.LIST_POSTID_LDA.get(i);
                            msgEnt.TopicId = ConfigInfo.LIST_TOPICID_LDA.get(j);
                            msgEnt.Probability = theta[i][j];
	...
	public boolean saveModelTwords(String filename){
        ...
        //ForDB
	public boolean saveModelTwordsDB(){
		try{
			if (twords > V){
				twords = V;
			}
			
                        ConfigInfo.LIST_TOPICID_LDA = new ArrayList<Long>();
                        for (int k = 0; k < K; k++){
                            ClassifiedTopicEnt newTopic = new ClassifiedTopicEnt();			
                            List<Pair> wordsProbsList = new ArrayList<Pair>(); 
                            for (int w = 0; w < V; w++){
                                    Pair p = new Pair(w, phi[k][w], false);
                            ...
                            long tmpTopicId = DataBaseService.CreateClassifiedTopic(newTopic);
                            ConfigInfo.LIST_TOPICID_LDA.add(tmpTopicId);
			} //end foreach topic			
